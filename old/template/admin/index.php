...
<section id="section-users">
    ...
    <table class="table table-striped table-bordered table-condensed">
        <thead>
        <tr>
            <th><?= l("admin.users.user"); ?></th>
            <th><?= l("admin.users.ticket"); ?></th>
            <th><?= l("admin.users.email"); ?></th>
            <th><?= l("admin.users.last"); ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        <? foreach (rpUserModel::select(["type" => "no"]) as $user): ?>
            ...
            <tr>
                <td><span
                        title="<?= str_replace("\"", "", $user['lastloginua']) . " " . $user['lastloginip']; ?>"><?= $user['uname']; ?>
                        (<?= $user['id']; ?>)</span></td>
                <td>
                    <a href="/admin/ticket/<?= $user['uname']; ?>/"><?= rpTicketModel::count(["uname" => $user['uname'], "status" => "ticket.status.open"]) ?></a>
                </td>
                <td><?= $user['email']; ?>(<?= $user['qq']; ?>)</td>
                <td><span
                        title="<?= gmdate(l("base.fullTime"), $user['lastlogintime']); ?>"><?= rpTools::niceTime($user['lastlogintime']); ?></span>
                </td>
                <td>
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="#"
                                   onclick="enableUser('<?= $user['uname']; ?>', 'std');"><?= l("admin.op.toStd"); ?></a>
                            </li>
                            <li><a href="#"
                                   onclick="enableUser('<?= $user['uname']; ?>', 'ext');"><?= l("admin.op.toExt"); ?></a>
                            </li>
                            <li><a href="#"
                                   onclick="enableUser('<?= $user['uname']; ?>', 'free');"><?= l("admin.op.toFree"); ?></a>
                            </li>
                            <li class="divider"></li>
                            <li><a href="#"
                                   onclick="deleteUser('<?= $user['uname']; ?>');"><?= l("admin.op.delete"); ?></a></li>
    ...
    <table class="table table-striped table-bordered table-condensed">
        ...
        <tr>
            ...
            <th><?= l("admin.users.email"); ?></th>
            <th><?= l("admin.users.last"); ?></th>
            <th><?= l("admin.users.expired"); ?></th>
            <th><?= l("admin.users.type"); ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        ...
        <? foreach ($db->query("SELECT * FROM `user` WHERE (`type`='free' AND `expired` > {$timeE}) OR (`type`!='no' AND `expired` < {$timeS} )") as $user): ?>
            <tr>
                ...
                <td><span
                        title="<?= gmdate(l("base.fullTime"), $user['expired']); ?>"><?= rpTools::niceTime($user['expired']); ?></span>
                </td>
                <td><?= l("base.userType")[$user['type']]; ?>
                    (<?= rpVirtualHostModel::count(["uname" => $user['uname']]); ?>)
                </td>
                <td>
                    <div class="btn-group">
                        <a class="btn dropdown-toggle btn-mini" data-toggle="dropdown" href="#">
                        ...
                        <ul class="dropdown-menu">
                            <li><a href="#"
                                   onclick="addTime('<?= $user['uname']; ?>');"><?= l("admin.op.addTime"); ?></a></li>
                            <li><a href="#"
                                   onclick="alertUser('<?= $user['uname']; ?>');"><?= l("admin.op.alert"); ?></a></li>
                            <li><a href="#"
                                   onclick="switchUser('<?= $user['uname']; ?>');"><?= l("admin.op.switch"); ?></a></li>
                            <li><a href="#"
                                   onclick="editSettings('<?= $user['uname']; ?>');"><?= l("admin.op.editSettings"); ?></a>
                            </li>
                            <?= $commonAct($user["uname"]); ?>
                        </ul>
                    </div>
                    ...
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li><a href="#"
                                   onclick="alertUser('<?= $user['uname']; ?>');"><?= l("admin.op.alert"); ?></a></li>
                            <li><a href="#"
                                   onclick="disableUser('<?= $user['uname']; ?>');"><?= l("admin.op.disable"); ?></a>
                            </li>
                            <li><a href="#"
                                   onclick="editSettings('<?= $user['uname']; ?>');"><?= l("admin.op.editSettings"); ?></a>
                            </li>
        ...
        <? foreach ($db->query("SELECT * FROM `user` WHERE `expired` > {$timeS} AND `type`!='no' AND `type`!='free'") as $user): ?>
            <tr>
                <td><span
                        ...
                        (<?= $user['id']; ?>)</span></td>
                <td>
                    <a href="/admin/ticket/<?= $user['uname']; ?>/"><?= rpTicketModel::count(["uname" => $user['uname'], "status" => rpTicketModel::OPEN]) ?></a>
                </td>
                <td><?= $user['email']; ?>(<?= $user['qq']; ?>)</td>
                <td><span
                        title="<?= gmdate(l("base.fullTime"), $user['lastlogintime']); ?>"><?= rpTools::niceTime($user['lastlogintime']); ?></span>
