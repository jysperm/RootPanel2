...
<? if($me->isAllowToPanel()): ?>
    <section id="section-access">
        ...
        <div>
            ...
            <button class="btn btn-success" onclick="changePasswd('mysql', false);"><?= l("panel.change.mysql");?></button>
        </div>
        <div>
            <input type="text" class="input-xxlarge" id="panelpasswd" name="panelpasswd"/>
            <button class="btn btn-success" onclick="changePasswd('panel', true);"><?= l("panel.change.panel");?></button>
        </div>
        <div>
            <input type="text" class="input-xxlarge" id="pptppasswd" name="pptppasswd"/>
            <button class="btn btn-success" onclick="changePasswd('pptp', false);"><?= l("panel.change.pptp");?></button>
        </div>
    </section>
    ...
    <section id="section-website">
        ...
        <? foreach(rpVirtualHostModel::select(["uname" => rpAuth::uname()]) as $host): ?>
            <div id="website<?= $host["id"];?>" class="box">
                ...
                <button class="btn btn-info pull-right" onclick="editWebsite(<?= $host["id"]; ?>);return false;"><?= l("form.change");?>
                </button>
            </div>
        <? endforeach; ?>
        <div class="box">
    ...
    <div>
        <table class="table table-striped table-bordered table-condensed">
            ...
            <? foreach(rpLogModel::select(["uname" => rpAuth::uname()], ["sort" => ["time", false], "limit" => 15]) as $log): ?>
                <tr>
                    <td><?= $log["id"]; ?></td>
                    <td><span title="<?= gmdate(l("base.fullTime"), $log["time"]); ?>"><?= rpTools::niceTime($log["time"]); ?></span>
                    </td>
